...
void BM_branched2_predicated(benchmark::State& state) {
    srand(1);
    const unsigned int N = state.range(0);
    std::vector<unsigned long> v1(N), v2(N);
    std::vector<int> c1(N);
...
BENCHMARK(BM_branch_not_predicated) ARGS;
BENCHMARK(BM_branch_predicated) ARGS;
...
BENCHMARK(BM_branched2_predicated) ARGS;
